...
REFERENCES S02.COMPANIES (ID) ON UPDATE NO ACTION;

ALTER TABLE S02.ITEM_COMPANIES ADD CONSTRAINT FK_ITEM_COMPANIES_ITEMS FOREIGN KEY (ITEM_ID)
REFERENCES S02.ITEMS (ID) ON UPDATE NO ACTION;
...
REFERENCES S02.ACCOUNTS (ID) ON UPDATE NO ACTION;
...
SELECT
    ...
    , t.TABSCHEMA AS HANDLER_SCHEMA
    , 'See Online Help' AS HANDLER_NAME
    , 'HTTP' AS HANDLER_TYPE
    , 'https://www.savetodb.com/samples/sample' || SUBSTRING(t.TABSCHEMA, 2, 2) || '-' || LCASE(t.TABNAME) || CASE WHEN USER LIKE 'sample%' THEN '_' || USER ELSE '' END AS HANDLER_CODE
...
WHERE
    t.TABSCHEMA = 'S02'
    AND NOT t.TABNAME LIKE 'XL_%'
UNION ALL
SELECT
    t.PROCSCHEMA AS TABLE_SCHEMA
    , t.PROCNAME AS TABLE_NAME
    , CAST(NULL AS VARCHAR) AS COLUMN_NAME
    , 'Actions' AS EVENT_NAME
    , t.PROCSCHEMA AS HANDLER_SCHEMA
    , 'See Online Help' AS HANDLER_NAME
    , 'HTTP' AS HANDLER_TYPE
    , 'https://www.savetodb.com/samples/sample' || SUBSTRING(t.PROCSCHEMA, 2, 2) || '-' || LCASE(t.PROCNAME) || CASE WHEN USER LIKE 'sample%' THEN '_' || USER ELSE '' END AS HANDLER_CODE
    , CAST(NULL AS VARCHAR) AS TARGET_WORKSHEET
    , 1 AS MENU_ORDER
    , 0 AS EDIT_PARAMETERS
FROM
...
WHERE
    t.PROCSCHEMA = 'S02'
    AND NOT t.PROCNAME LIKE 'XL_%'
    AND NOT t.PROCNAME LIKE '%_INSERT'
    AND NOT t.PROCNAME LIKE '%_UPDATE'
    AND NOT t.PROCNAME LIKE '%_DELETE'
    AND NOT t.PROCNAME LIKE '%_CHANGE'
    AND NOT t.PROCNAME LIKE '%_MERGE'
;
...
P1: BEGIN
    ...
    WHERE
        ...
        AND p.DATE BETWEEN COALESCE(USP_CASHBOOK2.START_DATE, '2010-01-01') AND COALESCE(USP_CASHBOOK2.END_DATE, '2038-01-19')
...
ELSEIF COLUMN_NAME = 'CHECKED' THEN

    UPDATE S02.CASHBOOK P
    SET
        CHECKED = CELL_NUMBER_VALUE
    WHERE
        P.ID = USP_CASHBOOK3_CHANGE.ID;
...
MERGE INTO S02.CASHBOOK AS t USING
    (VALUES (
        USP_CASHBOOK4_MERGE.ID
        , USP_CASHBOOK4_MERGE.DATE
        , USP_CASHBOOK4_MERGE.ACCOUNT_ID
        , USP_CASHBOOK4_MERGE.COMPANY_ID
        , USP_CASHBOOK4_MERGE.ITEM_ID
        , USP_CASHBOOK4_MERGE.DEBIT
        , USP_CASHBOOK4_MERGE.CREDIT
        , USP_CASHBOOK4_MERGE.CHECKED
        )
    ) s(ID, DATE, ACCOUNT_ID, COMPANY_ID, ITEM_ID, DEBIT, CREDIT, CHECKED)
ON t.ID = s.ID
WHEN MATCHED THEN
    UPDATE SET
        DATE = s.DATE
...
DECLARE item_id1 INTEGER;
DECLARE company_id1 INTEGER;
...
SET item_id1 = ITEM_ID;
SET company_id1 = COMPANY_ID;
SET year1 = YEAR;

SET month1 = INSTR('    Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dec ', ' ' || COLUMN_NAME || ' ') / 4;

IF month1 < 1 THEN SET month1 = NULL; END IF;

IF year1 IS NULL THEN SELECT YEAR(MAX("DATE")) INTO year1 FROM S02.CASHBOOK; END IF;

IF year1 IS NULL THEN SET year1 = YEAR(CURRENT_DATE); END IF;

SET start_date = DATE(CAST(year1 AS VARCHAR) || '-01-01') + (COALESCE(month1, 1) - 1) MONTH;

SET end_date = DATE(CAST(year1 AS VARCHAR) || '-01-01') + COALESCE(month1, 12) MONTH - 1 DAY;
...
INSERT INTO XLS.WORKBOOKS (NAME, TEMPLATE, DEFINITION, TABLE_SCHEMA) VALUES ('Sample 02 - Advanced Features - User1.xlsx', 'https://www.savetodb.com/downloads/v10/sample02-user1.xlsx',
